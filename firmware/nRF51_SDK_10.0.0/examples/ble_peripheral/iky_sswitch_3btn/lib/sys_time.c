...
#define DIFFERENCE_TIME		5 //haidv#26/05/2017
...
uint8_t ForcedUpdateRtcTime(uint32_t updateTimeSec)
{
...
{		
    if(rtcTimeSec < updateTimeSec)
		{
			if((updateTimeSec - rtcTimeSec) > DIFFERENCE_TIME)
				rtcUpdateFlag = 1;
		}
		else if(rtcTimeSec - updateTimeSec > DIFFERENCE_TIME)
			rtcUpdateFlag = 1;
		rtcUpdateCnt = updateTimeSec;
		return rtcUpdateFlag;
}
