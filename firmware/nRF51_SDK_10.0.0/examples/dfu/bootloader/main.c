...
#define LED_TOGGLE_TIMEOUT_INTERVAL			APP_TIMER_TICKS(1000, APP_TIMER_PRESCALER) 
...
static void leds_init(void)
{
	...
	nrf_gpio_cfg_output(LED_RED_1);
	nrf_gpio_cfg_output(LED_RED_2);
	nrf_gpio_cfg_output(LED_RED_3);
	nrf_gpio_cfg_output(LED_RED_4);
}
...
static void timers_init(void)
{
    ...
    // Start the DFU timer.
    err_code = app_timer_start(m_led_toggle_timer_id, LED_TOGGLE_TIMEOUT_INTERVAL, NULL);
    APP_ERROR_CHECK(err_code);
}
...
int main(void)
{
    ...
    //dfu_start  = app_reset;
		//dfu_start = true;
    dfu_start = ((bootModeFlag == BOOT_MODE_ACTIVE) ? true: false);
